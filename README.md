# XMediatorLevelPlayAppLovinAdapterCompatibility

Swift Package Manager package, published by X3M, that lets XMediator
integrations use the LevelPlay `AppLovinAdapter` adapter together with
the Google Mobile Ads mediation adapter for the same network.

## Why this package exists

The official LevelPlay adapter package (<https://github.com/ironsource-mobile/LevelPlay-AppLovin-Adapter-Swift-Package.git>) and the Google
mediation adapter package both declare a target named `AppLovinAdapter`,
...
`LevelPlayAppLovinAdapter`, renaming the conflicting module so both adapters can
coexist.
...
`5.7.1` of this package uses LevelPlay adapter `5.7.1`.
